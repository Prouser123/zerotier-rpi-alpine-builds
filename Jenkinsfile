...
node('docker-cli') {
  ...
  docker.image('jcxldn/jenkins-containers:base').inside {
    ...
    stage('Setup') {
	  ...
	  sh 'apk add make gcc build-base linux-headers && \
		  wget https://github.com/zerotier/ZeroTierOne/archive/${env.ZEROTIER_VERSION}.tar.gz -o zerotier.tar.gz && \
	...
	stage('Build') {
		sh 'cd ZeroTierOne-${env.ZEROTIER_VERSION}/ && \
